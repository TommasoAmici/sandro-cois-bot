...
const makeMatchesString = async (currentMatchday: number): Promise<string> => {
  ...
  const data = await api.get("/competitions/SA/matches/", { params });
...
export default (bot: TelegramBot, offset = 0) => async (
  ...
  if (currentMatchday === 0) bot.sendMessage(msg.chat.id, "Boh 🤷🏻‍♂️");
    ...
    try {
      ...
      bot.sendMessage(msg.chat.id, matchesString, { parse_mode: "Markdown" });
    } catch (error) {
